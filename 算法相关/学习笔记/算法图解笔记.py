...
第 11 章（接下来如何做）：
    1、树
        ...
        如果对数据库或高级数据结构感兴趣，可以研究如下数据结构：B树、红黑树、堆、伸展树
    ...
    5、MapReduce
        分布式算法
        MapReduce 基于两个简单的理念：映射函数（map） 和 归并函数（reduce）
    ...
    7、SHA 算法
        安全散列算法函数。给定一个字符串，SHA返回其散列值。
        用途：
            比较文件：判断两个文件是否相同
            检查密码

    8、局部敏感的散列算法
        Simhash 生成的散列值只存在细微的差别。能够让你通过比较散列值来判断两个字符串的相似程度。
